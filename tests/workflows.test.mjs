...
test("Phala deploys a compose file that names the image by digest", () => {
  // The compose text is what dstack hashes, and that hash is the only link
  // between a relying party's allowlist and the application. Delivering the
  // image as a CVM environment variable instead leaves the attested identity
  // covering the topology and nothing about which build is running.
  const workflow = parse(
    fs.readFileSync(`${directory}/phala-deploy.yml`, "utf8"),
  );
  const steps = workflow.jobs.deploy.steps;
  const render = steps.find((step) => step.name === "Render the compose file");
  const deploy = steps.find((step) => step.name === "Deploy CVM");
  const collect = steps.find(
    (step) =>
      step.name === "Collect deployment credentials and runtime configuration",
  );

  assert.ok(render, "the compose file must be rendered before the deploy");
  assert.match(render.env.IMAGE_DIGEST, /steps\.build\.outputs\.digest/);
  assert.match(render.env.IMAGE_VARIABLE, /composeVariable/);
  // By digest, never by the commit tag: a tag can be repointed after deploy,
  // which is the whole failure this closes.
  assert.match(render.run, /sha256:\[0-9a-f\]\{64\}/);
  assert.match(render.run, /IMAGE_REPOSITORY\}@\$\{digest\}/);

  // Silence is the danger: a placeholder that does not match must fail rather
  ...
  // The image must not also travel as an environment variable; two sources for
  // one fact is how they drift apart.
  assert.doesNotMatch(collect.run, /IMAGE_VARIABLE/);
  ...
  assert.equal(
    build.id,
    "build",
    "the digest is only available from an id'd step",
  );
  assert.ok(
    steps.indexOf(build) < steps.indexOf(render),
    "the image must be pushed before its digest is substituted",
  );
});
